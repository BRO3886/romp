...
Romp must independently run the verification contract before it opens a pull request, but it cannot know which project command expresses that contract. A repository can expose the command through a Makefile target, a package manifest, a CI workflow, or documentation, and project authors can use arbitrary names. A file-presence heuristic such as preferring `make test` over `npm test` can select the wrong command or select a command that does not exist.
...
Discovery is advisory. Romp limits discovery by file type, not by command name, and does not claim that a command is valid, available, or correct. It enumerates Makefile targets and package manifest scripts, includes exact commands found in CI workflows and Markdown documentation, merges duplicate commands, and displays the source references. It does not execute or expand wrappers and does not run a verification preflight during initialization.
...
- Candidate lists can be large and documentation or CI commands can be stale; the user remains responsible for the final selection.
